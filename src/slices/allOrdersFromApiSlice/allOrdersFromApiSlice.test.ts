...
describe('проверяем редьюсер allOrdersFromApiSlice', () => {
  test('обработка экшена fetchAllOrdersFromApi.pending', () => {
    ...
    const stateIsLoading = allOrdersFromApiSlice(
      initialState,
      action
    ).isLoading;
  ...
  test('обработка экшена fetchAllOrdersFromApi.rejected', () => {
    const action = {
      type: fetchAllOrdersFromApi.rejected.type,
      error: { message: 'test error fetchAllOrdersFromApi' }
    };
  ...
  test('обработка экшена fetchAllOrdersFromApi.fulfilled', () => {
    const action = {
      type: fetchAllOrdersFromApi.fulfilled.type,
      payload: {
