...
describe('проверяем редьюсер ordersUserSlice', () => {
  describe('проверка экшенов fetchOrderUser', () => {
    test('обработка экшена fetchOrderUser.pending', () => {
      ...
      const stateOrderRequest = ordersUserSlice(
        initialState,
        action
      ).orderRequest;
    ...
    test('обработка экшена fetchOrderUser.rejected', () => {
      const action = {
        type: fetchOrderUser.rejected.type,
        error: { message: 'test error fetchOrderUser' }
      };
    ...
    test('обработка экшена fetchOrderUser.fulfilled', () => {
      const action = {
        type: fetchOrderUser.fulfilled.type,
        payload: { order: { orderDone: 1 } }
      };
  ...
  describe('проверка экшенов fetchGetOrdersApiUser', () => {
    test('обработка экшена fetchGetOrdersApiUser.pending', () => {
      ...
      const stateRequestToApi = ordersUserSlice(
        initialState,
        action
      ).requestToApi;
    ...
    test('обработка экшена fetchGetOrdersApiUser.rejected', () => {
      const action = {
        type: fetchGetOrdersApiUser.rejected.type,
        error: { message: 'test error fetchGetOrdersApiUser' }
      };
      const stateError = ordersUserSlice(initialState, action).error;
    ...
    test('обработка экшена fetchGetOrdersApiUser.fulfilled', () => {
      const action = {
        type: fetchGetOrdersApiUser.fulfilled.type,
        payload: [{ order: 1 }, { order: 2 }, { order: 3 }]
      };
      const state = ordersUserSlice(initialState, action);

      expect(state).toEqual({
        historyOrdersUser: [{ order: 1 }, { order: 2 }, { order: 3 }],
        orderUser: null,
        requestToApi: false,
        orderRequest: false,
        error: undefined
